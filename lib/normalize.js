...
// Back-compat: some routes still import this. Keep it lightweight.
// It does NOT replace normalizeModelKey; it only smooths a few legacy artifacts.
export function degradeKeyForKnownBugs(k = '') {
  let s = String(k || '').toLowerCase().trim();
  if (!s) return '';

  // fix a couple of known historical artifacts
  s = s
    .replace(/\bzt\s+for\b/g, 'zt')   // "spider zt for" -> "spider zt"
